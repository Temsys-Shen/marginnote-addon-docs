...
当前样本环境中`CATransition`、`CAAnimationGroup`、`CAPropertyAnimation`类均可见，且都存在`new/alloc/animation`入口。字段是否“可写且生效”仍以具体动画场景验证为准。
...
| `filter` | id | filter。 |
| `keyPath` | string \| null | keyPath（运行时复用字段）。 |
| `fromValue` / `toValue` / `byValue` | any | 起始/结束/增量值（运行时复用字段）。 |
| `values` | NSArray | values（运行时复用字段）。 |
...
| `animations` | NSArray | 子动画数组（运行时复用字段）。 |

## 类成员 (Class members)

运行时未额外暴露 `CATransition` 的类成员。
