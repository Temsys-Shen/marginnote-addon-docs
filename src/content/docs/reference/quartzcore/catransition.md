...
`CATransition` 继承自 [CAAnimation](/reference/quartzcore/caanimation/)，用于描述层的转场动画（例如淡入淡出、推入等）。插件环境暴露了常见动画配置字段。

## 接口状态

`CATransition`按`CATransition.animation()`创建。
...
| `filter` | id | filter。 |
| `keyPath` | string \| null | keyPath（复用字段）。 |
| `fromValue` / `toValue` / `byValue` | any | 起始/结束/增量值（复用字段）。 |
| `values` | NSArray | values（复用字段）。 |
...
| `animations` | NSArray | 子动画数组（复用字段）。 |

## 类成员 (Class members)

当前接口未额外暴露 `CATransition` 的类成员。
