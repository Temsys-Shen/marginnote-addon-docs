...
`CAShapeLayer` 继承自 [CALayer](/reference/quartzcore/calayer/)，用于渲染矢量路径的描边与填充。插件环境的 `path`/`fillColor`/`strokeColor` 等字段以“宿主对象”形式暴露，你可以把它们当作可传递的值来设置与读取。
