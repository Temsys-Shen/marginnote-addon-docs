...
当前样本环境中`UICollectionViewLayout`类本身可见，但直接构造路径不稳定。实践上建议优先通过现有`UICollectionView`实例读取`collectionViewLayout`，不要在插件里直接新建布局实例。

在插件里做首版网格列表时，建议优先使用`UICollectionViewFlowLayout`并设置固定`itemSize/sectionInset/minimumLineSpacing`。`sizeForItemAtIndexPath`这类动态尺寸路径在JS桥接里风险较高，建议后置。
