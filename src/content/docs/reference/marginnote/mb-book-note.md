...
| `excerptTextMarkdown` | `number` | 摘录文本是否为 Markdown（运行时字段，为 0/1）。 |
...
| `addChild(note)` | 添加子节点。 |
| `processMarkdownBase64Images()` | 处理 Markdown 中的 base64 图片（由运行时定义）。 |
| `sortCommentsByNewIndices(newIndices)` | 按新索引数组重排 comments。 |
| `getDrawingSize()` / `getStrokesCount()` | 绘制相关信息（由运行时定义）。 |
